...
public:
  ...
  void frugal_accum_update(const int16_t sample) {
    ...
    // if (sample < pedestal_) --accum_;
    if (sample > pedestal_ && accum_ < std::numeric_limits<int16_t>::max()) {
      accum_ += 1;
    }
    if (sample < pedestal_ && accum_ > std::numeric_limits<int16_t>::min()) {
      accum_ -= 1;
    }

    if (accum_ > accum_limit_ &&
        pedestal_ < std::numeric_limits<int16_t>::max()) {
      pedestal_ += 1;
      accum_ = 0;
    }
    if (accum_ < -1 * accum_limit_ &&
        pedestal_ > std::numeric_limits<int16_t>::min()) {
      pedestal_ -= 1;
      accum_ = 0;
    }
  }
  ...
  void process_waveform(
    ...
    for (size_t i_t = 0; i_t < adcs.size(); ++i_t) {
      ...
      if (is_over) {
        ...
        tmp_charge += sample;
        tmp_charge =
            std::min(tmp_charge, (int32_t)std::numeric_limits<int16_t>::max());
        ...
        if (hit_tover_ < std::numeric_limits<int16_t>::max()) {
          hit_tover_ += 1;
        }
      }
