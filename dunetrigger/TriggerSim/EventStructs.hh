...
namespace duneana
{
  ...
  struct tpEvent
  {
    ...
    tpEvent(TDCTime_t tdc_time, TDCTime_t peak_time, uint32_t adc_int, uint32_t adc_peak) : tdc_time(tdc_time),
                                                                                            peak_time(peak_time),
                                                                                            adc_int(adc_int),
                                                                                            adc_peak(adc_peak)
    {
  ...
  struct electronEvent
  {
    ...
    electronEvent(TDCTime_t tdc_time, float num_electrons, float energy) : tdc_time(tdc_time),
  ...
  struct pulse_t
  {
    ...
    pulse_t(TDCTime_t tdctime, TDCTime_t peak_time, uint32_t charge, float energy, uint32_t channel, uint32_t peak_charge) : tdctime(tdctime),
                                                                                                                             charge(charge),
                                                                                                                             energy(energy),
                                                                                                                             channel(channel),
  ...
  {
  public:
    static std::map<raw::ChannelID_t, std::vector<tpEvent>> process_tps_into_map(std::vector<dunedaq::trgdataformats::TriggerPrimitive> &tps)
    {
    ...
    static std::map<raw::ChannelID_t, std::vector<electronEvent>> process_es_into_map(std::vector<sim::SimChannel> &electrons)
    {
    ...
    static std::vector<pulse_t> channel_pulse_charges(std::vector<electronEvent> &channel_data, uint32_t threshold, uint32_t channel)
    {
